...
while True:
   ...
   if grabResult.GrabSucceeded():
      ...
      # param1=90, param2=40, minRadius=35, maxRadius=65 
      )
      ...
      if detected_circles is not None and len(detected_circles[0]) > 0:
            ...
            # Display the smoothed robot coordinates
             ...
             if name_text == "Robot Coordinates" and start_time is 0 :
                   start_time = current_time  
             elapsed_time = current_time - start_time if start_time is not 0 else 0
            ...
            #  time.sleep(5)
             while (name_text == "Robot Coordinates" and elapsed_time >=5.0):
               ...
               #    dType.SetPTPCmd(api, dType.PTPMode.PTPMOVLXYZMode, x_robot_smoothed, y_robot_smoothed, 0, -15, isQueued=1)
               #    dType.SetWAITCmd(api, 200, isQueued=1)
               #    dType.SetPTPCmd(api, dType.PTPMode.PTPMOVLXYZMode, x_robot_smoothed, -162, 0, -15, isQueued=1)
               #    dType.SetWAITCmd(api, 200, isQueued=1)
               dType.SetPTPCmd(api, dType.PTPMode.PTPMOVLXYZMode, -2.2, -162, 0, -15, isQueued=1)
               dType.SetWAITCmd(api, 200, isQueued=1)
               #    dType.SetPTPCmd(api, dType.PTPMode.PTPMOVLXYZMode, -2.2, -162, 70, -15, isQueued=1)
               #    dType.SetWAITCmd(api, 200, isQueued=1)
               #    dType.SetPTPCmd(api, dType.PTPMode.PTPMOVLXYZMode, 99, -180, 70, -15, isQueued=1)
               #    dType.SetWAITCmd(api, 200, isQueued=1)
               #    dType.SetPTPCmd(api, dType.PTPMode.PTPMOVLXYZMode, 175, -100, 70, -15, isQueued=1)
               #    dType.SetWAITCmd(api, 200, isQueued=1)
               #    dType.SetPTPCmd(api, dType.PTPMode.PTPMOVLXYZMode, 175, -100, 70, -15, isQueued=1)
               #    dType.SetWAITCmd(api, 200, isQueued=1)
               ...
               dType.SetWAITCmd(api, 200, isQueued=1)
               #    dType.SetPTPCmd(api, dType.PTPMode.PTPMOVLXYZMode, 175, 57, 70, -15, isQueued=1)
               #    dType.SetWAITCmd(api, 200, isQueued=1)
